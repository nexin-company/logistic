...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${LOGISTIC_API_URL}/v1/stock/adjust`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.message || 'Error al ajustar stock' },
    ...
    console.error('Error en POST /api/logistic/v1/stock/adjust:', error);
    return NextResponse.json(
